...
void
skiplist_init(struct skiplist *list)
{
	list->height = 0;
...
__device__ __host__ static inline struct skiplist_node *
skiplist_next(struct skiplist *list, struct skiplist_node *n, int k)
{
	return (struct skiplist_node *) list_next(&list->heads[k], n, list[k]);
	
}

__device__ __host__ struct skiplist_node *
skiplist_find(struct skiplist *list, const void *key,
...
void
skiplist_link(struct skiplist *list, struct skiplist_node *n,
...
void
skiplist_erase(struct skiplist *list, struct skiplist_node *n,
		    struct skiplist_node *update[SKIPLIST_LEVELS])
{
	for (int k = 0; k <= list->height; k++) {
		struct skiplist_node *next = (struct skiplist_node *)skiplist_next(list, update[k], k);
