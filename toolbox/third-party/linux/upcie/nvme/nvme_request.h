...
/**
 * Build PRP2 / the PRP-list for a contiguous data buffer that spans more than one page.
 *
 * Out-of-line slow path for nvme_request_prep_command_prps_contig(). Kept
 * non-inlined so the per-entry hostmem_dma_v2p() translations are not emitted
 * into the single-page fast path, where they perturb its codegen. The `inline`
 * keyword here governs only header linkage (the definition may appear in many
 * translation units, and is dropped without a -Wunused-function warning when a
 * unit does not use it); __attribute__((noinline)) forces the out-of-line
 * emission. PRP1 is expected to already be set by the caller.
 */
static inline void __attribute__((noinline))
nvme_request_prep_command_prps_contig_multipage(struct nvme_request *request,
						struct hostmem_heap *heap, void *dbuf,
				      ...
				      void *dbuf, size_t dbuf_nbytes, struct nvme_command *cmd)
{
	...
	if (page_off + dbuf_nbytes > heap->config->pagesize) {
		nvme_request_prep_command_prps_contig_multipage(request, heap, dbuf, page_off,
								dbuf_nbytes, cmd);
	}
}
