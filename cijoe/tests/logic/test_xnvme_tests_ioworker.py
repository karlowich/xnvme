...
def test_verify_iovec(cijoe, device, be_opts, cli_args):
    ...
    # large nlb
    err, _ = cijoe.run(f"xnvme_tests_ioworker verify {cli_args} --nlb 3 --vec-cnt 4")
...
def test_verify_sync_iovec(cijoe, device, be_opts, cli_args):
    ...
    # large nlb
    err, _ = cijoe.run(
        f"xnvme_tests_ioworker verify-sync {cli_args} --nlb 3 --vec-cnt 4"
    )
...
def test_verify_iovec_direct(cijoe, device, be_opts, cli_args):
    ...
    # large nlb
    err, _ = cijoe.run(
        f"xnvme_tests_ioworker verify {cli_args} --nlb 3 --vec-cnt 4 --direct 1"
    )
...
def test_verify_sync_iovec_direct(cijoe, device, be_opts, cli_args):
    ...
    # large nlb
    err, _ = cijoe.run(
        f"xnvme_tests_ioworker verify-sync {cli_args} --nlb 3 --vec-cnt 4 --direct 1"
    )
