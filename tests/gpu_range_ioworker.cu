...
struct iowork {
	...
	uint32_t *slbas;   ///< First LBA of the range
	uint32_t *elbas;   ///< Last LBA of the range
...
iowork_pp(struct iowork *work)
{
	...
	for (uint32_t i = 0; i < work->n_ranges; i++) {
		printf("  slbas[%" PRIu32 "]: %" PRIu32 "\n", i, work->slbas[i]);
		printf("  elbas[%" PRIu32 "]: %" PRIu32 "\n", i, work->elbas[i]);
	}
...
iowork_from_cli(struct xnvme_cli *cli, struct iowork *work)
{
	...
	cudaMallocManaged(&work->slbas, sizeof(uint32_t));
	cudaMallocManaged(&work->elbas, sizeof(uint32_t));
...
test_verify(struct xnvme_cli *cli)
{
	...
	for (int i = 0; i < 2; i++) {
		...
		err = xnvme_kernels_range_submit(x, y, work->dev, opc, work->slbas, work->elbas, work->io.nlb, work->io.nbytes, cur_bufs, work->n_ranges);
		if (err) {
			return err;
		}
	}
