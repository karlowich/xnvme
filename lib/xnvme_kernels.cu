...
__global__ void
_range_submit2(struct xnvme_dev *dev, uint32_t opc, uint32_t *slbas, uint32_t *elbas, uint32_t nlb, uint64_t nbytes, void **dbufs, uint32_t n_ranges, uint32_t n_threads, uint32_t range)
{
	void *buf;
	uint32_t slba = slbas[range];
	uint32_t	elba = elbas[range];
	void *	dbuf = dbufs[range];
	uint32_t cur_slba;
	uint64_t tid;
	uint64_t i = 0;
	while(true) {
...
xnvme_kernels_range_submit(uint32_t grid_size, uint32_t tblock_size, struct xnvme_dev *dev, uint32_t opc, uint32_t *slbas, uint32_t *elbas, uint32_t nlb, uint64_t nbytes, void **dbufs, uint32_t n_ranges)
{
	...
	uint32_t gs = grid_size / n_ranges;
	for (uint32_t i = 0; i < n_ranges; i++) {
		_range_submit2<<<gs, tblock_size>>>(dev, opc, slbas, elbas, nlb, nbytes, dbufs, n_ranges, gs * tblock_size, i);
	}
	err = cudaGetLastError();
	if (err != cudaSuccess) {
		XNVME_DEBUG("Error launching kernel: %s", cudaGetErrorString(err));
